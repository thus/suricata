...
static int TLSParseHandshakeHello(SSLState *ssl_state, uint8_t *input,
                                  uint32_t input_len, SSLStateConnp *curr_connp)
{
    ...
    while (processed_len < extensions_len)
    {
        ...
        switch (ext_type) {
            case SSL_EXTENSION_SNI:
            {
                ...
                curr_connp->sni = SCMalloc(sni_strlen);

                if (unlikely(curr_connp->sni == NULL))
                    goto end;

                memcpy(curr_connp->sni, input, sni_strlen - 1);
                curr_connp->sni[sni_strlen-1] = 0;
                                   ...
                                   uint32_t input_len)
{
    ...
    switch (ssl_state->curr_connp->handshake_type) {
        case SSLV3_HS_CLIENT_HELLO:
            ...
            rc = TLSParseHandshakeHello(ssl_state, input, input_len,
                                        &ssl_state->client_connp);
            if (rc < 0)
                return -1;
