...
static SSLCipherSuite g_suites[] = {
...
SSLCipherSuite * SSLCipherSuites(size_t *len) {
    ...
    *len = sizeof(g_suites) / sizeof(SSLCipherSuite) - 1;
    return g_suites;
}
...
static int TLSDecodeHandshakeHello(SSLState *ssl_state, uint8_t *input, uint32_t input_len)
{
    ...
    if (ssl_state->curr_connp->handshake_type == SSLV3_HS_CLIENT_HELLO) {
        uint32_t num_cipher_suites = input[0] << 8 | input[1];
        input += 2;

        if (num_cipher_suites > 0) {
            ssl_state->curr_connp->num_cipher_suites = num_cipher_suites / 2;
            ssl_state->curr_connp->cipher_suites = SCMalloc(num_cipher_suites);
            if (ssl_state->curr_connp->cipher_suites) {
                memcpy(ssl_state->curr_connp->cipher_suites, input, num_cipher_suites);
            } else {
                ssl_state->curr_connp->num_cipher_suites = 0;
            }
            input += num_cipher_suites;
        }
