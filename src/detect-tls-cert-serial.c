...
void DetectTlsSerialRegister(void)
{
    ...
    sigmatch_table[DETECT_AL_TLS_CERT_SERIAL].AppLayerMatch = NULL;
    sigmatch_table[DETECT_AL_TLS_CERT_SERIAL].Setup = DetectTlsSerialSetup;
    sigmatch_table[DETECT_AL_TLS_CERT_SERIAL].Free = NULL;
    ...
    sigmatch_table[DETECT_AL_TLS_CERT_SERIAL].flags |= SIGMATCH_PAYLOAD;

    DetectMpmAppLayerRegister("tls_cert_serial", SIG_FLAG_TOCLIENT,
            DETECT_SM_LIST_TLSSERIAL_MATCH, 2,
...
{
    s->list = DETECT_SM_LIST_TLSSERIAL_MATCH;
...
static int DetectTlsSerialTest01(void)
{
    ...
    sm = de_ctx->sig_list->sm_lists[DETECT_SM_LIST_TLSSERIAL_MATCH];
    FAIL_IF_NULL(sm);
