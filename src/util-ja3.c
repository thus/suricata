 ...
 * \retval -1 on failure.
 */
static int Ja3BufferResizeIfFull(JA3Buffer *buffer, unsigned len)
{
    if (buffer == NULL) {
    ...
    {
        buffer->size *= 2;
        buffer->data = SCRealloc(buffer->data, buffer->size * sizeof(char));
        if (buffer->data == NULL) {
...
int Ja3BufferAppendBuffer(JA3Buffer *buffer1, JA3Buffer *buffer2)
{
    ...
    if (buffer2->used == 0) {
        buffer1->used += sprintf(buffer1->data + buffer1->used, ",");
    } else {
        buffer1->used += sprintf(buffer1->data + buffer1->used, ",%s",
                                 buffer2->data);
    }
...
int Ja3BufferAddValue(JA3Buffer *buffer, int value)
{
    ...
    if (buffer->used == 0) {
        buffer->used += sprintf(buffer->data, "%d", value);
    }
    else {
        buffer->used += sprintf(buffer->data + buffer->used, "-%d", value);
    }
